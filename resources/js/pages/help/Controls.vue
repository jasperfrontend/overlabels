              ...
              <div class="mt-6" id="available-functions">
                ...
                <div class="space-y-4 text-sm text-foreground">
                  ...
                  <div>
                    ...
                    <p>Standard math functions. <code class="rounded bg-background px-1 py-0.5 font-mono text-xs">max</code>, <code class="rounded bg-background px-1 py-0.5 font-mono text-xs">min</code>, <code class="rounded bg-background px-1 py-0.5 font-mono text-xs">sum</code>, and <code class="rounded bg-background px-1 py-0.5 font-mono text-xs">avg</code> accept multiple arguments. <code class="rounded bg-background px-1 py-0.5 font-mono text-xs">round</code> takes an optional decimals count: <code class="rounded bg-background px-1 py-0.5 font-mono text-xs">round(0.1 + 0.2, 2)</code> returns the string <code class="rounded bg-background px-1 py-0.5 font-mono text-xs">"0.30"</code> (trailing zero preserved, matching the <Link href="/help/formatting" class="text-violet-400 hover:underline">|round:2 pipe</Link>). Because the 2-arg form returns a string, put it at the end of the expression; further math after it concatenates instead of adding.</p>
                  </div>
                  <div>
                    <div class="mb-1.5 flex flex-wrap gap-1.5">
                    ...
                    <p>Animation-friendly helpers. <code class="rounded bg-background px-1 py-0.5 font-mono text-xs">sin</code> and <code class="rounded bg-background px-1 py-0.5 font-mono text-xs">cos</code> take radians. <code class="rounded bg-background px-1 py-0.5 font-mono text-xs">fract(x)</code> returns the fractional part (<code class="rounded bg-background px-1 py-0.5 font-mono text-xs">x - floor(x)</code>, so <code class="rounded bg-background px-1 py-0.5 font-mono text-xs">fract(-0.3) === 0.7</code>). <code class="rounded bg-background px-1 py-0.5 font-mono text-xs">mod(a, b)</code> is floor-based modulo (GLSL-style), so <code class="rounded bg-background px-1 py-0.5 font-mono text-xs">mod(-1, 5) === 4</code>. Use the <code class="rounded bg-background px-1 py-0.5 font-mono text-xs">%</code> operator if you want JS remainder (<code class="rounded bg-background px-1 py-0.5 font-mono text-xs">-1 % 5 === -1</code>). <code class="rounded bg-background px-1 py-0.5 font-mono text-xs">PI</code> is a bare identifier - use <code class="rounded bg-background px-1 py-0.5 font-mono text-xs">PI</code>, not <code class="rounded bg-background px-1 py-0.5 font-mono text-xs">PI()</code>.</p>
                    <p class="mt-2">Heads up on float precision: <code class="rounded bg-background px-1 py-0.5 font-mono text-xs">fract(10.2)</code> evaluates to <code class="rounded bg-background px-1 py-0.5 font-mono text-xs">0.19999999999999993</code>, not <code class="rounded bg-background px-1 py-0.5 font-mono text-xs">0.2</code>. That's expected - it's how IEEE 754 doubles work, same in every language (JavaScript, GLSL, Python, C). For animation math the trailing noise is invisible. For text display, pipe through a <Link href="/help/formatting" class="text-violet-400 hover:underline">formatter</Link> like <code class="rounded bg-background px-1 py-0.5 font-mono text-xs">|round:2</code> to get a clean <code class="rounded bg-background px-1 py-0.5 font-mono text-xs">0.2</code>.</p>
                  </div>
                  <div>
                    <div class="mb-1.5">
