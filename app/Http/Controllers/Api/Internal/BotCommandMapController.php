...
class BotCommandMapController extends Controller
{
    /**
     ...
     * POST to the fire endpoint for custom commands.
     *
     * Shape:
     *
     *   {
     ...
     *       { "command": "distance", "permission_level": "everyone",   "type": "custom"         },
     ...
     *   builtin > custom > alias > recipe_trigger > list_append > list_meta.
    ...
    public function index(): JsonResponse
    {
        $users = User::where('bot_enabled', true)
            ...
            ->with(['botBuiltins' => fn ($q) => $q->where('enabled', true)])
            ->get();

        $customByUser = BotCommand::where('enabled', true)
        ...
        foreach ($users as $user) {
            ...
            foreach ($customByUser->get($user->id, collect()) as $expr) {
