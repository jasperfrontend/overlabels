...
- Scopes: `socket.token`, `donations.read`. `donations.create` was requested from the start and never called - dropped in OL-2609-097, because asking for a write permission we have no code to use makes the consent screen meaningless. Do not re-add it.
...
- **Prod migrated off Cloudinary on 2026-08-18 and the importer has been deleted** (it was entrypoint scaffolding with an explicit removal trigger - don't reintroduce a permanent one). `docs/private/Cloudinary_Archive_*.zip` is NOT a usable restore source: it is flat, prefix-stripped and `-1`-suffixed on collisions, so nothing in it maps back to a `public_id`.
- **Local dev was deliberately NOT migrated** - `overlabels.test` may still render Cloudinary URLs, and local uploads fail safely (caught, "Upload failed"). **Never point a local install at the prod images bucket**: `deleteByUrl()` checks the LOCAL DB for references, so a local delete would destroy an object prod still uses. A local setup needs its own bucket.
...
- `/help/integration-presets` stays Inertia deliberately (live data from `controlPresets.ts`); it uses `AppLayout` directly. Do not "finish the job" by converting it.
- **Chat Castle is gone (2026-09-08, OL-2609-034).** The chat-driven dungeon game shipped in April 2026, was shelved in May on art direction, and was removed entirely: seven `Game*` models and their tables, `app/Services/Gamejam`, the `gamejam.*` routes, the `/help/gamejam` Vue page and `HelpLayout.vue`, the room builder, 1300 tracked asset files, and the six bot verbs `join`, `p`, `h`, `a`, `s`, `castlehelp` from `BotBuiltin::DEFAULTS` plus their `bot_builtins` rows. Do not rebuild it, and do not propose it as a chat-game base; the `!checkin` integration is the shape a chat feature takes now.
...
- Per-message tags: `author`, `login`, `text`, `html`, `color`, `badges`, `badge_images`, `at`, `id`, `mod`, `sub`, `vip`, `broadcaster`, `first`, `source_channel`, plus `chat.count`. Index 0 is OLDEST. `at` is Unix seconds.
- **The window size is a foreach cap like any other**: `foreach_caps.chat` in user preferences, set on `/settings/account`, default 50, ceiling `FOREACH_CAP_MAX`. It is the ONLY cap enforced client-side - the other four slice a Twitch payload in `TemplateDataMapperService` before the overlay sees it, but chat has no server-side array because the overlay reads Twitch directly. It ships as `chat_window` in the render payload and becomes the socket's window size via `setWindowSize()`. `clampWindow()` must fall back to the DEFAULT on NaN, not to 1: `Number(json.chat_window)` from an older server is NaN and would otherwise pin every overlay to a one-message feed.
