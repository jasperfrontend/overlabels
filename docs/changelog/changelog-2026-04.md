...
- Public surface lives at `/updates` (list, paginated 15) and `/updates/{slug}` (single post), both inside the `auth.redirect` group so they sit alongside `/templates` rather than being globally public. `UpdateController::index` accepts `search` (case-insensitive title + excerpt + body LIKE), `tag` (`whereJsonContains` against the tags array), and `from`/`to` date-range filters; the URL stays canonical via the same strip-empties `buildQuery()` pattern as `/templates`. The list page also receives `allTags` (distinct tag set across all published updates) so the tag filter is a real dropdown rather than a free-text input. Filter state is preserved across browser back/forward via the same `watch(props.filters)` re-sync used by the templates page.
- Admin surface at `/admin/updates` with full CRUD on `AdminUpdateController` (`admin.role` middleware). Create/edit share `resources/js/pages/admin/updates/edit.vue` - one form for both, distinguished by a nullable `update` prop. Title and slug are plain inputs (slug optional - defaults to `Str::slug(title)`); tags use a comma-separated input that's split on submit and validated as `string|max:64` per tag; excerpt is a plain textarea; the body uses the existing `vue-codemirror` integration with `lang-html` (markdown allows inline HTML, so HTML highlighting is the closest reasonable mode without adding `@codemirror/lang-markdown` to the deps); `published_at` is a `datetime-local` input that defaults to "now". Every create/update/delete writes an `AdminAuditService` entry with the slug + title in the metadata so the audit log can reconstruct intent later.
- New `UpdatesList.vue` component mirrors `TemplateList.vue`'s visual pattern (`<Link>`-wrapped row, `Heading` with title + excerpt, kebab `DropdownMenu` aligned right, `cursor-pointer` on every clickable). Action set is intentionally narrower than templates: "Read post" + "Copy link" for everyone, plus "Edit" + "Delete" gated behind an `isAdmin` prop. The two list components are *not* merged - templates' kebab carries fork/preview/OBS-link actions that don't apply to blog posts, and the refactor risk on a working component outweighed the DRY win for this scope. Both components stay independent for now; if a third list shows up later we'll extract a shared row layout then.
- Single-post page (`updates/show.vue`) borrows the help-page layout (`max-w-4xl`, hero h1, breadcrumbs `Updates > {title}`) and renders the rendered-markdown body inside an `article.prose` block with scoped `:deep(.prose ...)` styles for typography (h1-h4, lists, code blocks, blockquotes, images). Tags render as Inertia `<Link>`s back to `/updates?tag={tag}` so clicking a tag filters the list. Admins get a small "Edit" button next to the "All updates" back link.
- Dashboard wiring: `DashboardController` loads up to 5 most recent published updates and the dashboard grid grows a "Recent updates" section that uses the new `UpdatesList` and reuses `DashboardSectionHeader`. The header's "View all" link goes to `/updates`; admins also get a `+` button that links to `/admin/updates/create`. Section is hidden entirely when there are no updates yet so a fresh install doesn't show an empty card.
- Admin sidebar: a new "Updates" entry (`Newspaper` icon) is appended after "Lockdown" in `adminSectionItems`, keeping the existing convention that admin-only nav items only render when the user is on an admin page.
- `Update` interface added to `resources/js/types/index.d.ts` with the same shape used by both the public list, the public show page, and the admin form. Tags are `string[] | null`; the `published_at` is the ISO string Inertia serializes from the datetime cast.

## April 29th, 2026 - Move alert/event assignment into each alert template's "Triggers" tab; retire the global Alerts Builder

- The standalone `/alerts` "Alerts Builder" page never fit the rest of the UI. Its job was to map every Twitch (and Ko-fi/StreamLabs/StreamElements/Fourthwall) event type to one alert template, with per-row duration + transition fields. But alert templates already own their own `templates/{id}/edit` page with tabs for Code, Meta, Tags, Controls, Values, Screenshot, and Targeting (which static overlays an alert fires onto). It made no sense for "which events trigger this alert" to live anywhere except next to "which overlays this alert lands on". This commit moves that responsibility onto the template edit page as a new "Triggers" tab (alert-only, `Zap` icon, sits left of Targeting) and reduces `/alerts` to a read-only overview moved into the user dropdown menu under Integrations.
- New endpoint `PUT /templates/{template}/triggers` (Inertia, redirect-back) on `OverlayTemplateController` accepts `{ twitch: [{event_type, duration_ms, enabled}], external: [{service, event_type, duration_ms, enabled}] }` and replaces this template's mappings only. Logic: rows where `template_id = $template->id` (or `overlay_template_id` for external) but missing from the request body are deleted; rows in the body are upserted with `template_id = $template->id`. Mappings owned by other templates of the same user are never touched. Reassigning a Twitch event from another template to this one transparently overrides the previous owner because of the existing `(user_id, event_type)` unique index - that's intentional and matches how the old global UI behaved.
- The edit/show controllers preload `triggers` for alert templates: full Twitch `EVENT_TYPES` catalogue, per-service external event-type catalogue, the user's connected services list, and the assigned-to-this-template subset (twitch + external). Same payload shape on both routes via a new private `buildTriggerData()` helper to keep show + edit aligned.
- New `TriggerManager.vue` component: a single Save button at the top, then one `Collapsible` group per source (Twitch always present, plus one group per connected external service - matches the grouping pattern from `ControlsManager.vue` / `ControlPanel.vue`). Each row has just two controls: an enabled toggle and a duration input. Expanded/collapsed state per group is persisted to `localStorage` under `triggers_manager_expanded`. The Twitch group icon is `Zap` (violet); external groups carry the same orange "external" pill the alerts page used.
- Transitions are gone, with prejudice. The `transition_in` / `transition_out` columns on `event_template_mappings` and `external_event_template_mappings` are dropped (`2026_04_29_000001_drop_transition_columns_from_event_template_mappings`); `AlertTriggered` no longer broadcasts them; `OverlayRenderer.vue` drops the wrapping `<transition>` element, the `activeTransitionName` ref, and the `@leave` cleanup hook (cleanup moved inline into `dismissAlert`); the constants on both mapping models are deleted; the duration-only validator on the new endpoint accepts `1000..999000` (the old global UI capped at 30s). Reason: the wrapper transitions never composed cleanly with author-written CSS animations inside the alert HTML, and they made alerts flicker on layout reflow. Authors keep full control by writing their own `@keyframes` inside the alert template's `head`/`css`.
- Duration cap raised from 30 to 999 seconds. Three-digit max in the UI input (`Math.min(999, Math.max(1, ...))`); the new endpoint validates `1000..999000` ms. Long-form alerts (poll progress, hype train, charity campaigns) routinely run longer than 30s.
- `events/index.vue` (still at `/alerts`) is now a read-only overview: each Twitch event currently bound to an alert template renders as a card linking to `templates.show` for that alert; same for each connected external service; an "Unassigned Twitch events" pill cloud at the bottom shows event types not yet bound anywhere. The page is reachable from `UserMenuContent.vue` ("Event alerts overview", `Megaphone` icon) directly below "Integrations". The "Alerts builder" entry in `AppSidebar.vue` is removed.
- Cleanup: `ExternalEventTemplateMappingController` deleted (the `events.external.*` and `events.update.bulk` route group is gone; nothing else referenced the controller). `EventTemplateMappingController` is now `index()`-only (read-only overview). The Ziggy `except` list is pruned of the four obsolete route names. The `OnboardNewUser` job's auto-assign step no longer writes transition fields. `AlertTargetingTest` updated to drop `transition_in`/`transition_out` from its fixture mapping creates.
- New `TemplateTriggersTest`: 8 tests covering create, delete-on-omission, no-cross-talk between templates, override-on-reassignment, external-service writes, 403 for non-owners, 422 for non-alert templates, and the new 999000 ms upper bound.

## April 29th, 2026 - Build the `/dashboard/stream-sessions` UI on top of the new aggregator

- The page now ships a real overview UI instead of the JSON dump. Each stream is one card; cards are listed most-recent first. Card layout: header row (date, start-end times, duration, completed/live badge), an "approximate window" warning row (visible only when `window.anchored_on_eventsub.online` or `.offline` is false - tells the streamer the stats fall back to `started_at`/`ended_at` because no matching `stream.online`/`stream.offline` event was found), a 2-3-6-column responsive headline tile grid (follows, new subs with tier breakdown, resubs with cumulative months, gift subs, raids in with viewers, redemptions with points spent), the latest title strip showing the most recent `channel.update` snapshot plus a "N title changes" hint when there's history, and a "Show details" toggle that reveals the per-section deep dive.
- Detail sections (each rendered only when there's data, so quiet streams stay quiet): title and category timeline (left-bordered list), polls (per-poll card with status badge, `truly_finished`/"awaiting archive" indicator, total votes, per-choice horizontal bars showing vote share - the winner choice is highlighted amber with a Trophy icon), hype trains (level + total + type + top contributors), goals (start->end / target progress bar with delta), recent resub messages (name + tier badge + months + cumulative/streak + the message in italics), incoming raids list, channel-point redemptions by reward (table with reward title, count, cost-per, total cost), cheers (only if any), all event counts (the raw `event_counts` map as a wrap of small monospace badges - useful for spotting events the controller doesn't aggregate yet, e.g. predictions), and a `<details>` capture-window debug panel for cross-checking which timestamps anchor the window.
- Theming follows the project conventions: every container border uses `border border-sidebar-border`, body copy is `text-foreground` and only metadata/hints are `text-muted-foreground`, the toggle button uses the `.btn .btn-plain .btn-sm` styles from `overlabels-buttons.css`, every clickable has `cursor-pointer` (via the `.btn` class). Polls' "winner" highlight uses amber-400; goal progress uses green-500/70; non-winning poll choices use violet-400/70 to keep the existing brand accent. No em dashes anywhere - hyphens with spaces only.
- Type-safety: the `StreamSession` interface in `stream-sessions.vue` is fully typed against the controller's actual JSON shape (poll choices, hype train top contributors, goal progress, resub messages, raids, reward breakdown). The previous shape that landed in this file was speculative and didn't match the real payload (`recent_messages.tier` was typed as number, `raids` was typed as a single object instead of an array, `polls` was missing winners and lifecycle); this commit overwrites it with the correct shape so future template edits get real autocomplete instead of guesses.
- Composable usage: `const { userLocale, formatDate, formatTime } = useSessionDataFormatter()`. No `speedUnit` prop needed for stream sessions - the optional argument design pays off immediately.

## April 29th, 2026 - Convert `useSessionDataFormatter` into a real composable (fixes prod crash on `/dashboard/gps-sessions`)

- The previous shape of `resources/js/composables/useSessionDataFormatter.ts` had `defineProps<{ speedUnit: string }>()` called at module top level, plus `usePage()` and `computed()` also at module scope. `defineProps` is a Vue compiler macro that only gets transformed inside `<script setup>` blocks of `.vue` files - in a `.ts` file the compiler ignores it and the literal `defineProps()` call gets shipped to runtime, where it's `undefined` and throws `ReferenceError: defineProps is not defined` the moment the module is imported. Verified by grepping the built bundle: `public/build/assets/gps-sessions-*.js` contained literal `z=defineProps()` before this fix, meaning `/dashboard/gps-sessions` would crash on page load on any deploy that included the broken composable. Build + lint did not catch it because the macro looks like a normal function call to ESLint and Vite. (After this change: zero `defineProps` occurrences in the gps-sessions bundle.)
- The `usePage()` and `computed()` at module scope are also idiomatically wrong even though they happen to work with Inertia v2: they create a single global computed effect that lives for the entire app lifetime, never gets cleaned up by an `effectScope`, and triggers Vue's dev-mode "computed outside of a setup or scope" warning. The conventional fix is the composable factory pattern - call `useFoo()` from setup, get a per-instance scope.
- `useSessionDataFormatter` rewritten as `export function useSessionDataFormatter(opts: { speedUnit?: Ref<string> } = {})` - returns `{ userLocale, formatDate, formatTime, formatDuration, formatDistance, formatAltitude, formatSpeed, batteryDelta, batteryColor }`. `speedUnit` is optional (so stream-sessions.vue can use the locale/date/duration helpers without needing a unit ref) and reactive (formatters re-compute when the user toggles the setting). `usePage()` and the `userLocale` computed now live inside the factory so they bind to the calling component's effect scope.
- Side-fix while touching the file: `formatSpeed` and `formatAltitude` returned the literal string `'-'` for null inputs, which violates the project rule "render nothing when data is missing - no dashes, no N/A, no invented fallbacks". Both now return `null`. Vue's interpolation renders `null` as empty string, so the existing template at `gps-sessions.vue` displays "avg " instead of "avg -" when there are no pings - which is what the rule wants.
- Wired `gps-sessions.vue` to the new shape: `const { formatSpeed, formatDistance, ... } = useSessionDataFormatter({ speedUnit: toRef(props, 'speedUnit') })`. `toRef(props, 'speedUnit')` keeps the ref reactive across props updates without losing reactivity through destructuring. `stream-sessions.vue` now imports the composable but doesn't destructure anything yet - a comment shows the destructure pattern for when the actual UI gets built.

## April 29th, 2026 - Dedupe `channel.poll.end` in stream-sessions overview

- Twitch fires `channel.poll.end` twice per poll: once with `status` of `completed` (poll ended naturally) or `terminated` (mod/streamer stopped it early), then again with `status: archived` after a brief delay. The two payloads are otherwise identical (same id, same final votes, same winners). On the new `/dashboard/stream-sessions` overview this surfaced as misleading event counts - CasualElephant's stream with one poll showed `channel.poll.end: 2` even though only one poll actually ended.
- Two-part fix in `StreamSessionController`: in `loadHeadlineAggregates`, the `event_counts` LATERAL subquery now uses `CASE WHEN event_type = 'channel.poll.end' THEN COUNT(DISTINCT event_data->>'id') ELSE COUNT(*) END` so the per-event-type count map dedupes poll.end by poll id while every other event type stays a raw COUNT. In `loadBoundedListEvents`, the per-poll collapse was rewritten around an explicit snapshot-priority + lifecycle-flags model: priority resolves to `resolved-end (3) > archived-end (2) > begin (1)` so the final snapshot always carries the real vote counts regardless of arrival order, while three independent flags (`_has_begin`, `_has_end_resolved`, `_has_end_archived`) are updated on every event seen. Output adds `truly_finished` (true only when both `has_end_resolved` AND `has_end_archived`) and a `lifecycle` object exposing the raw flags so the future UI can distinguish "running", "ending" (resolved or archived but not both), and "finished" states.
- Robust to arrival quirks the user flagged: archived may arrive before completed/terminated, either may be delayed, and either may never show up at all. The previous `_locked` mechanic locked the snapshot on the first completed end seen and then ignored everything else for that poll - which would have masked the missing-archived case as "fully done". The new flags-track-everything model preserves the truth.
- Prod verification: re-ran the new aggregate against CasualElephant's session - `channel.poll.end` count dropped from 2 to 1 as expected. Other event-type counts are unchanged.

## April 29th, 2026 - Per-stream stats overview at `/dashboard/stream-sessions` (backend wiring only)

- The `stream_sessions` table has been quietly accumulating one row per stream since the state machine landed in early April, but nothing exposes it back to the streamer. New route `GET /dashboard/stream-sessions` aggregates per-session statistics from `twitch_events` so the streamer can see what actually happened during each broadcast. The Vue page is intentionally a raw JSON dump for now - the goal of this commit is to land the backend wiring and let the data shape settle before any UI gets designed on top of it.
- Capture window per session is anchored on the EventSub `stream.online` and `stream.offline` events themselves rather than `stream_sessions.started_at` / `ended_at`. Reason: the state machine's `started_at` is Helix-corrected (the value Helix reports as the true stream start), which can lag the EventSub `stream.online` notification by several minutes during OBS warmup. CasualElephant's April 29th stream is the canonical example - `stream.online` fired at 03:41:58, the session opened at 03:51:05, and the streamer changed the title at 03:40:50. Anchoring on `stream.online` and adding a 5-minute pre-buffer catches that pre-stream `channel.update`; anchoring on `started_at` would miss it. A 30-minute search window from the session bounds finds the matching anchor events; if no anchor is found (older sessions, mismatched events) the controller falls back to `started_at` / `ended_at` and flags `window.anchored_on_eventsub.{online,offline} = false` in the response so the UI can mark those sessions as approximate.
- Stat aggregation is fully SQL-side to keep this flat regardless of stream size. A naive "pull every event in the window into Eloquent and aggregate in PHP" approach blows up on big streamers - 5k channel-point redemptions per stream times 50 sessions in the list is 250k rows hydrated into models for what's ultimately a `COUNT(*)`. Instead, the controller fires seven scoped queries total (regardless of session count or per-session event count): a windows-resolution CTE; a headline aggregate using `COUNT(*) FILTER` and `SUM((event_data->>'x')::int) FILTER` for follows/subs/resubs/gift-subs/raids/cheers/redemptions plus a `jsonb_object_agg` for the per-event-type counts map; subs grouped by tier; redemptions grouped by reward title (bounded by the streamer's reward catalog, typically <30); goals first/last per type via `(array_agg(... ORDER BY ts ASC))[1]` (same trick `GpsSessionController` uses for battery start/end); a single bounded-list query for `channel.update`/`channel.raid`/`channel.poll.{begin,end}`/`channel.hype_train.end` only (poll.progress is excluded because the begin/end snapshots already include cumulative choices); and a resub-messages query capped at 25 most recent per session via `ROW_NUMBER() OVER (PARTITION BY session_id ORDER BY ts DESC)`. Verified on prod against CasualElephant's two sessions: headline aggregate executes in 0.75ms across 181 in-window events.
- API shape note for the future drill-down route: per-event lists for unbounded event types (follows, cheers, channel-point redemptions) are deliberately not in the response - just counts and bucketed breakdowns (`by_tier`, `by_reward`). When the per-session detail page lands later it should be a separate paginated route that pulls the raw event log on demand. Resub messages survive in the overview but capped at 25 per session.
- Files: new `app/Http/Controllers/StreamSessionController.php`; new Vue stub at `resources/js/pages/dashboard/stream-sessions.vue` that renders `JSON.stringify(sessions, null, 2)` inside the standard `AppLayout` shell with `Radio` icon and "Stream Sessions" breadcrumb; route registration and import added to `routes/web.php` next to the existing GPS sessions routes. No sidebar nav entry yet - the page is reachable by URL only until the UI is built.
...
- `app/Http/Controllers/TwitchEventSubController.php`: new private helper `computePollWinners()` returns every choice tied at the max `votes` value (bits and channel-points votes are deliberately ignored - those are weighting inputs, not the visible tally). `handleTwitchEvent()` injects `$event['winners']` for the three poll types right after `$event = $data['event']`, so the same enriched payload flows into `TwitchEvent::create(['event_data' => $event])`, `renderEventAlert($data)`, and `broadcast(new TwitchEventReceived($event))`. One compute, three exposures, no drift.
- DSL surfacing: `TemplateDataMapperService::INDEXED_LIST_FIELDS` and `OverlayTemplate::FOREACH_EVENT_CAPS` both got a `'winners' => 5` entry. The first makes the existing indexed-flatten branch produce `event.winners.count`, `event.winners.0.{id,title,votes,...}`, and `event.winners.total_votes`; the second makes `extractTemplateTags()` expand `[[[foreach:event.winners as winner]]]` into the concrete keys so the per-template allowlist doesn't strip them. The frontend foreach reconstruction in `useConditionalTemplates.ts` needed no change - it already rebuilds arrays from any flattened indexed prefix, and `[[[raw]]]` inside the loop already dumps the scoped item as JSON.
- All-zero edge case: with no votes yet, every choice ties at 0 so all choices come through as winners. This is intentional - it lets a single alert template handle begin, progress, and end uniformly without status-based branching. Documented in the three reference pages (`poll-started.md`, `poll-progress.md`, `poll-ended.md`).
- Tests: 3 new mapper-flattening tests in `EventSubExpansionTest.php` (single winner, tie, all-zero), 1 new tag-extraction test in `TemplateTagExtractionTest.php`, and a new `TwitchPollWinnersTest.php` unit suite covering the helper directly via reflection (single winner, tie, all-zero, empty input, ignores bits/channel-points votes, returns full choice objects, returns a re-indexed list so `array_is_list()` passes in the mapper).
- Follow-up fix to `mapForTemplate`'s allowlist prune: `[[[raw]]]` inside a foreach was dumping a stripped-down item containing only the subkeys the body referenced by name (e.g. `{"id":"c1"}` for a body that only wrote `[[[winner.id]]]`). Cause: `extractTemplateTags()` only allowlists the explicitly-referenced subkeys, and `[[[raw]]]` doesn't reference anything specifically. Fix: when the allowlist contains `<iterable>.count` (always emitted by `extractForeachTags`), keep every key with that prefix - so the reconstructed item the frontend hands to `[[[raw]]]` has the full shape (id, title, votes, bits_votes, channel_points_votes for poll choices). New test in `EventSubExpansionTest.php` locks this in.
- Second follow-up in `useConditionalTemplates.ts::resolveIterable()`: a foreach over a user-capped iterable (e.g. `followed_channels` with the per-user cap set to 5 in `/settings/appearance` while the user actually follows 24 channels) was running the loop 24 times - 5 real items followed by 19 empty `{}`. Cause: the function used `<iterable>.count` (source-of-truth size, 24) as the loop length, padding missing indices. Pre-existing bug, only visible once `[[[raw]]]` started rendering something for every iteration. Fix: cap iteration at the highest actually-populated index so the loop only runs over data we have. `count` itself is unchanged for `[[[<iterable>.count]]]` display use.

## April 28th, 2026 - Restore Twitch/BTTV/FFZ/7TV emote rendering in alerts (without reopening the v-html injection surface)

- Regression introduced by the April 13th security commit (`security: HTML-encode substituted tag values in OverlayRenderer`): every substituted tag value is now passed through `encodeHtml()` before landing in the alert HTML. That fix did not distinguish between donor-supplied raw text and the safe `<img>` HTML that `useEmoteParser` deliberately produces, so `Hello Kappa` in a channel-point redeem rendered with a literal `&lt;img ...&gt;` instead of the emote image.
- The naive fix (flip alert substitution back to `encode: false`) would re-open the exact hole the April 13th commit closed - a viewer typing `<img src="https://evil/track.gif">` in their redeem text would render that tag in the streamer's overlay. The real fix has to keep encoding-by-default for tag values while letting the emote parser's output bypass it.
- `resources/js/composables/useEmoteParser.ts`: `parseToken()` now HTML-encodes the token before it ever reaches the BTTV/FFZ/7TV parser library, and the Twitch position-based path encodes the emote name used as the `<img alt="...">`. Encoded text still contains alphanumeric emote codes (`Kappa`, `LUL`, etc.), so the parser library's `[a-zA-Z0-9_-]+` matcher continues to find and replace them with safe `<img>` markup. A user typing `<img src=evil>Kappa` becomes `&lt;img src=evil&gt;Kappa` (encode pass) -> `&lt;img src=evil&gt;<img alt="Kappa" src="...cdn..." ...>` (emote pass). The `<img src=evil>` is inert text; the `Kappa` is a real emote.
- `resources/js/components/OverlayRenderer.vue`: `compiledAlertHtml` gained a small pre-substitution pass for an explicit allowlist of html-safe fields (`event.message.text`, `event.user_input`). After conditionals are processed but before `replaceTagsWithFormatting` runs, those `[[[event.user_input]]]` / `[[[event.message.text]]]` tokens are spliced directly into the HTML, then deleted from the data object passed to the encoded substitution pass. Net effect: emote-parsed fields skip encoding (their parser already encoded the dangerous chars), every other tag is still encoded as before. Allowlist is opt-in by string match - any future field that needs the same treatment gets one entry in `HTML_SAFE_ALERT_FIELDS`.
- Conditional logic on these fields still works because `processTemplate` runs against the raw `alertData` first; only the substitution data is filtered.
- Verified the threat model still holds: a donor sending the literal string `<img src="https://example.com/img.png" alt="haha" />` as a Ko-fi/StreamLabs/StreamElements message renders as visible escaped text, not an image. A viewer typing the same string in a channel-point redeem also renders as escaped text. Only known emote codes (Twitch, BTTV, FFZ, 7TV) produce real `<img>` tags, and their `src` URLs come from the channel's emote sets, never from user input.

## April 28th, 2026 - Per-page OG images for /help/reference, rendered via resvg

- Until now, every help/reference page (135 entries + the index) shared the same generic Cloudinary OG image. Once the reference moved off Inertia and started ranking individually in Google, the same preview card showing up under every link unfurled in Slack/Discord was a missed branding opportunity. Now each entry gets its own card with the entry's title, category eyebrow, code snippet (where applicable), body excerpt, and canonical URL footer baked in.
- Render path is `Blade SVG -> resvg -> PNG`, no headless Chrome. The SVG (`resources/views/og/help-reference.blade.php`) keeps the layout from the static design mockup, parameterised with five slots: eyebrow, title, optional code label/snippet, body lines (up to 4, greedily wrapped at ~75 chars per line, ellipsised on overflow), and footer URL. Pixel-identical 1200x630 output to what the design started from.
- New `App\Services\OgImageService` builds the per-page context from the existing `HelpReferenceService` entry shape, hashes it with a `TEMPLATE_VERSION` constant, and stores the rendered PNG at `public/og/{sha256}.png`. `urlFor($entry, $canonical)` and `urlForIndex($total, $canonical)` are the two entry points. Cache is content-addressable: edit a markdown body, the hash changes, the next request renders fresh; bump `TEMPLATE_VERSION` (currently 2) and every page invalidates at once. Render failures (resvg missing, fonts missing, anything thrown by the Process) log a warning and fall back to `/ogimage.png` so a broken render never takes the page meta down.
- Variant logic without a config panel: `template-tags` slugs render `[[[slug]]]` in the code slot; `foreach-loops` render `[[[foreach:slug]]]`; aggregate `all-*` list pages count their bullet markers and show "N entries"; the index page shows total count; `eventsub-events` and `eventsub-tags` (no snippet) render title + body + URL only. The body excerpt strips markdown headings (any level), fenced code blocks, Obsidian-style `[[slug]]` and `[[slug|label]]` links (keeping labels), `[[[tag]]]` markers, inline-code backticks, and leading list bullets, then collapses whitespace - so what makes it onto the OG card reads as prose, not raw markdown.
- New artisan command `php artisan og:generate [--force]` pre-renders the index plus every reference entry. Running on dev: 135 PNGs in ~3 seconds, 5 MB total in `public/og/` (gitignored). The earlier proof-of-concept command `php artisan og:test` is kept for one-off SVG-to-PNG smoke checks.
- Wired through `HelpReferenceController::show()` - the controller injects `OgImageService` and computes `$ogImage` (entry URL or index URL) which the layout consumes. `resources/views/layouts/help.blade.php` now resolves `$ogImage` to either an absolute URL (if it starts with `http`) or runs it through `url()`, falling back to the legacy Cloudinary URL if no `$ogImage` is set so non-reference pages using this layout keep working unchanged. Also added `og:image:type=image/png` and pulled `og:image:alt` from `$pageTitle` instead of the static brand line.
- Fonts: dropped 23 `.ttf` files into `resources/fonts/` (Albert Sans family + Fira Code Regular through Bold). resvg discovers them via `--use-fonts-dir resources/fonts`. The SVG references `'Albert Sans'` and `'Fira Code'` as family names so the Regular weight resolves correctly.
- Production wiring (Kamal/Docker, not GitHub):
    - `Dockerfile` Stage 3 now downloads pinned `resvg-linux-x86_64.tar.gz` v0.47.0 from the official `linebender/resvg` GitHub release into `/usr/local/bin/resvg` and verifies with `resvg --version` so a bad URL fails the build instead of silently shipping a broken image.
    - `docker/docker-entrypoint.sh` adds an `ENTRYPOINT_RUN_OG_GENERATE` gate that runs `php artisan og:generate` once per container boot (mirroring the existing `ENTRYPOINT_RUN_MIGRATIONS` pattern). Failures log a warning and continue - the layout's Cloudinary fallback covers any rendering breakage.
    - `config/deploy.yml` sets `ENTRYPOINT_RUN_OG_GENERATE=1` on the `web` role only. Queue, scheduler, and Reverb roles share the same image but skip the render so they don't waste boot time on PNGs they'll never serve.
- Boot-time cost on prod is roughly 3-5 seconds for the initial render. Subsequent boots short-circuit on `File::exists` per hash, so unchanged entries cost a stat call each. Entries whose body changed between deploys re-render on the boot after the change.
- One known cosmetic: pages without a code snippet (eventsub-events, eventsub-tags) leave a vertical gap where the snippet block would sit because the body block's y-position is fixed in the SVG. Legible but a little airy. Two ways to address later: shift the body up when no snippet, or synthesize a snippet for those pages (the EventSub event key would do it). Out of scope for the first cut.
- Markdown content note: some existing reference markdown contains em-dashes inherited from prior content. Per project policy new copy uses spaced hyphens, but the OG renderer doesn't auto-rewrite existing source, so the legacy characters show up verbatim in body excerpts where present. Cleanup is a content task for another day, not a renderer concern.
...
- Replaced the Inertia render with a Blade-rendered route. `HelpReferenceController::show` reads `resources/help/reference/{category}/{slug}.md` from disk, parses with `league/commonmark` (already a transitive dep), and returns `resources/views/help/reference.blade.php`. Real `<title>`, `<h1>`, `<h2>`, real `<a href>` between entries, all `[[[tag]]]` snippets in the response body. Each entry has its own canonical URL with full server-rendered HTML.
- The two custom markdown transforms from the JS pipeline (`useHelpReference.ts`) are reproduced in PHP: Obsidian-style `[[slug]]` and `[[slug|label]]` wikilinks resolve against a slug-to-category map and become real `/help/reference/{category}/{slug}` links (or fall back to inline code if the slug is unknown), and triple-bracket `[[[tag]]]` snippets become click-to-copy `<code class="ov-tag" data-tag="..." role="button">` widgets. Both transforms run outside fenced and inline code spans so authors can show literal syntax without it being rewritten. Title extraction reuses the same `^# Title` heuristic the JS side used.
- Sidebar tree is rendered server-side by Blade from `HelpReferenceService::grouped()`. File scan is cached for 60 minutes keyed on the directory mtime - editing a `.md` file invalidates the cache automatically. Service order matches the original (`template-tags`, `eventsub-tags`, `eventsub-events`, `foreach-loops`).
- Search is preserved as a progressive enhancement. New artisan command `php artisan help:build-index` walks every entry once and writes `public/help-reference-index.json` (62.9 KB, served gzipped). Wired into composer's `post-autoload-dump` so any deploy that runs `composer install/update` regenerates it. The output is gitignored.
- New tiny Vite entry `resources/js/help-reference/main.ts` (~2 KB excluding fuse.js) handles all the interactive bits: fetches the index JSON, wires fuse.js fuzzy search against the same key weights the Vue page used, click-to-copy on tag widgets and the snippet header copy button, transient toast on copy, sidebar scroll-position persistence in `localStorage`, Alt+R to focus the search input, and an external-link interceptor that adds `target="_blank" rel="noopener noreferrer"` on outbound anchors inside the rendered prose.
- The page-scoped CSS from `Reference.vue` (the `.help-prose` and `.ov-tag` styles, plus `:deep(...)` markdown typography) is flattened into `resources/js/help-reference/styles.css` since the page no longer runs through Vue's scoped-CSS pipeline.
- New layout `resources/views/layouts/help.blade.php` is intentionally bare: dark-mode preference detection inline (matches `app.blade.php`), font preconnect, dynamic `<title>` / `<meta description>` / `<link rel="canonical">` / OG + Twitter meta from controller variables, a minimal navbar (logo top-left, "Help" + "Back to app" top-right). No AppLayout, no sidebar, no Echo, no codemirror, no leaflet. The page is not behind auth - anyone (including Googlebot) can read it.
- Inertia compatibility: `<Link>` and `router.visit()` calls elsewhere in the app that target `/help/reference` would normally trip Inertia's "non-Inertia response" debug modal once they got HTML back. The controller detects the `X-Inertia` header and returns a 409 with `X-Inertia-Location` set to the same URL, which makes Inertia's client do a clean `window.location` reload. Direct browser nav and external referrers see the 200 HTML response unchanged.
- Removed: `resources/js/pages/help/Reference.vue` (the Inertia version). Kept `resources/js/composables/useHelpReference.ts` because `ReferencePalette.vue` (the global Alt+R modal inside the auth'd app) still uses it for in-memory fuzzy search across the loaded markdown bundle. Bundle-size win is therefore partial - the public reference URL stops shipping the corpus, but the auth'd palette still does. A follow-up could have the palette also consume `/help-reference-index.json` to drop the glob import, but that's out of scope here.
- Companion fix in `vite.config.mts`: `npm run build:ssr` was failing with `"vue-codemirror" cannot be included in manualChunks because it is resolved as an external module`. Vite's SSR build externalizes `node_modules` by default, so any package listed in `manualChunks` produces a contradiction. Switched `defineConfig({...})` to the function form `defineConfig(({ isSsrBuild }) => ({...}))` and only apply `manualChunks` when `isSsrBuild` is false. Client build still gets chunked codemirror/websocket/leaflet bundles; SSR build gets a clean `output: {}`. SSR is not yet enabled in production - it's available if/when the rest of the app gets audited for browser-globals access.
- New Vite entry registered: `resources/js/help-reference/main.ts` added to the `laravel-vite-plugin` `input` array so `@vite(['resources/css/app.css', 'resources/js/help-reference/main.ts'])` resolves both in dev (Vite hot) and prod (manifest).
