...
## April 26th, 2026 - Cloudinary replace flow no longer orphans the previous asset

- Hole in the April 25th hardening: `ImageDropZone.vue`'s "Replace image" affordance let users swap an existing image for a new one in one drop, but the previous Cloudinary asset was being left behind on the CDN. The Remove button worked correctly (`updateScreenshot` saw `screenshot_url=null` and called `deleteByUrl`), but the replace path was skipping the cleanup in practice - rediscovered abuse vector: upload, replace, repeat = unbounded asset accumulation under a single claimed `cloudinary_uploads` row.
- Closed by binding the cleanup to the upload event itself rather than the parent's subsequent persist call. `POST /cloudinary/upload` now accepts an optional `replaces_url`. When the dropzone already holds an image and the user drops/pastes/picks a replacement, the frontend sends the previous URL alongside the new file. The backend, after a successful new upload, calls `deleteByUrl($previousUrl)` with the appropriate exclude based on `kind` - finds the user's own template (or kit) that still points at the URL and excludes it from the "is anything still referencing this?" check, so the existing fork-share guard still prevents accidental deletes of URLs that other templates/kits depend on.
- `ImageDropZone.vue`: captures `props.modelValue` at the start of `uploadToCloudinary` and appends it to `FormData` as `replaces_url` if set. No changes to any callers (`TemplateScreenshot.vue`, `kits/create.vue`, `kits/edit.vue`) - the field is detected from the model value the dropzone already exposes.
- Why bind to the upload, not the parent persist: the previous design relied on `OverlayTemplateController::updateScreenshot` (and similar in kits) to detect old-vs-new and call `deleteByUrl`. That logic is still in place and still correct, but it executes only after the parent's PUT lands and Inertia re-syncs. The new upload-level cleanup runs synchronously in the same request as the upload itself, which means the cleanup happens regardless of whether the parent's PUT succeeds, fails, or never fires (the kit-create flow defers persistence until form submit). Belt and suspenders.
- Existing `CloudinaryUploadServiceTest` (6 tests, 9 assertions) still green - the deletion logic in `CloudinaryUploadService::deleteByUrl` is unchanged, only the trigger point is broadened.
