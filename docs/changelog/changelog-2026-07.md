...
- **A "Your CSS and fonts" panel** under the canvas, on both `/builder` and the edit page's Code tab. Optional, collapsed by default, so the no-code on-ramp is untouched for anyone who doesn't want it. One `BuilderStylePanel.vue` serves both surfaces, and the state lives in `useBuilderState`, so the two save paths needed no new plumbing.
- **The class list is harvested from the blocks you actually placed** (`utils/builderClasses.ts`), so you pick a real target instead of guessing at someone else's markup. Classes the block styles itself sort first, since those are the ones worth overriding, and the rest collapse behind "show N unstyled" - that's what stops a Tailwind-authored block from burying `.value` under sixty utility classes. Classes built from tags (`class="text-[[[c:color]]]"`) are skipped, because there's nothing stable to target. Clicking one appends an empty rule. `.builder-cell`, the wrapper class the compiler puts on every placement, always leads the list as the handle for "all of them at once".
- **Custom CSS is scoped to `#builder-root`, not appended raw**, and this is the whole reason the feature works. A block's `.value` rule compiles to `#blk-a3f2 .value`, specificity (1,1,0). A bare `.value` at the bottom of the file is (0,1,0) and loses no matter how late it appears, so the obvious implementation would have silently done nothing for precisely the classes people most want to change. Run through the existing `prefixCss` it becomes `#builder-root .value`, which ties on specificity and wins on source order. The same holds at any depth: match the selector the block used and you win. It also keeps one promise true in both directions - you write plain CSS, Overlabels scopes it - which is already what the Builder tells block authors.
- **A `<head>` field for fonts**, appended after the deduped block heads so a Google Fonts link plus one `font-family` rule restyles a whole composition. Never deduped against the blocks' own head tags: re-declaring a font the blocks already pull in is a legitimate thing to want, and later wins anyway.
- **Both live in `metadata.builder`, not the css/head columns.** They have to: `composeBuilderTemplate` rebuilds head/html/css from scratch on every save, so anything typed into those columns would be destroyed by the next one. Being compile input rather than compile output also means they survive ejecting to the code editor, and `[[[c:accent_color]]]` works inside them for free, since template tags already resolve in CSS.
- **Fixed a latent keyboard bug the panel exposed.** Typing in the new CSS editor made the block you had just placed vanish after a few seconds. The Builder's canvas shortcuts (arrows move, Shift+arrows resize, Delete/Backspace remove) bail out when the event targets an input, but the test was `['INPUT', 'TEXTAREA', 'SELECT'].includes(target.tagName)` and CodeMirror renders a **contenteditable `<div>`**, so every keystroke reached the canvas. Placing a block selects it, so the block being styled was always the one under the cursor: the first Backspace deleted it, and arrow keys silently moved it around the grid instead of moving the caret. The guard is now `isTextEntryTarget()` in `utils/`, which is the check `useKeyboardShortcuts` was already doing correctly - that layer now imports it too, so the rule lives in one place instead of being re-derived per handler.
- **Tests:** 4 new in `BuilderTemplateTest` covering the round-trip, the empty default, script stripping in the custom head, and the 65535 length cap. Full file 15 passed. `vue-tsc` and ESLint clean.
...
- **The admin override was seeding the wrong control entirely.** It wrote the amount into `donations_received`, which counts how many donations arrived rather than what they were worth, so an admin "correcting" someone's seed to 65.35 was quietly setting their donation *count* to 65 while the total they had actually set sat untouched. Same class of bug that was found and fixed elsewhere in the app a while back; the admin panel was the last place still carrying it. It now calls the same `seedTotalReceived()` the user-facing path uses, so the two cannot drift apart again. Pinned by a test that asserts the value lands in `total_received` and leaves `donations_received` at zero - verified to fail on the old key for all six services.
- **It also had to accept decimals**, or this change would have broken it: the form pre-fills from `donations_seed_value` and posts it straight back, so the first user to seed 65.35 would have made that panel un-saveable.
- **Throne was missing from the admin panel** in both `SEEDABLE_SERVICES` and the route's service constraint, so its seed was the one that could not be corrected. Added to both.
- **Corrected copy that named the wrong control.** Every one of the six settings pages, and the admin card, said the seed feeds your `donations_received` controls. It does not and never did. Headings and the test-mode reset warning now say "total" rather than "count", which is also what the value is.
- 48 tests across the six services: a fractional amount lands on the control, a whole one stays whole, three-decimal precision and negatives are refused, locale notation is refused at the boundary on purpose, the admin override hits the total and not the counter, and a seeded 65.35 survives a test-mode round trip - the one path that resets service-managed controls and has to put the seed back rather than zero.
...
The cause was one line. `OverlayTemplateFactory::definition()` declared `'fork_of_id' => OverlayTemplate::factory()` - a self-referential foreign key whose default is a nested factory, so it has no base case and recurses without limit. Because Laravel resolves nested factory attributes in declaration order, `'owner_id' => User::factory()` fires first at every level and commits a user, then `fork_of_id` recurses before the template it belongs to is ever inserted. So a single `OverlayTemplate::factory()->create()` produces users forever and templates never. Reproduced with a counter: one call, 21 users, 0 templates, 261 stack frames at depth 21. At 19,684 levels that is roughly 240,000 nested frames, which is also why the insert rate decayed from 100/s to 6/s before the process fell over.

- **`fork_of_id` now defaults to `null`**, which is what a nullable self-referential FK should have been doing all along, with a `->forked()` state for the cases that actually want a parent. The comment on the default says why, in the file where the next person will be standing.
- **`FactoryRecursionTest`** pins the base case: one template and exactly one user per `create()`, a null `fork_of_id` by default, `->forked()` producing a parent that does not itself recurse, and `OverlayControl::factory()` not recursing through its template FK. That last one was a live landmine - `OverlayControlFactory` nests `OverlayTemplate::factory()` and no test had called it yet.
- **The test suite was never the culprit.** All 38 `OverlayTemplate::factory()` call sites across 21 files already passed `'fork_of_id' => null`, per the warning in `CLAUDE.md`. Every author remembered the workaround, which is precisely why the trap survived: it only fires for ad-hoc calls, and it fired from a `tinker` session that was seeding verification data by hand. Twelve seconds before the first junk user, Telescope logged a `tinker` invocation; three minutes into the runaway, another `tinker` built a template with raw `OverlayTemplate::create([...])` and every column spelled out. Someone hit this wall in June and routed around it without ever seeing what it was.
- 19,759 rows deleted after confirming all 33 foreign keys referencing `users.id` had zero dependent rows, and that none of the targets carried an access token, Twitch data, an admin role, or a system-user flag. Full suite green at 1,103 passed.
...
- **Nothing pointed at the file.** It was not linked from the document head, not mentioned in `robots.txt`, and not in `sitemap.xml`. Anything that did not already know the URL had no way to find it. Added to all three. The head link uses `rel="llms-txt"` rather than `rel="alternate"`, because llms.txt is not an alternate representation of whatever page you happen to be on and saying so would be a lie to any crawler that understands the relation. The root path stays the actual contract; these are hints on top of it. Note the link has to go in **two** files: `app.blade.php` covers every Inertia page, but `/` is served from the standalone `welcome` view and inherits nothing from the shell, so the first version shipped with the link on `/help` and `/login` and not on the homepage - the one page a crawler reaches first. Caught by checking production rather than assuming the shell was global.
